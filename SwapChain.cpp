...
SwapChain::SwapChain(HWND hwnd, UINT width, UINT height, RenderSystem* system) : m_system(system)
{
	...
	if (FAILED(hr))
		throw std::exception("Failed to create {SwapChain}. Error in ctor / {m_dxgi_factory->CreateSwapChain(...)}.");
...
void SwapChain::reloadBuffers(unsigned int width, unsigned int height)
{
	...
	if (FAILED(hr))
	{
		throw std::exception("Failed to create {SwapChain}. Error in ctor / {m_swap_chain->GetBuffer(...)}.");
	}
	...
	if (FAILED(hr))
	{
		throw std::exception("Failed to create {SwapChain}. Error in ctor / {device->CreateRenderTargetView(...)}.");
	}
	...
	if (FAILED(hr))
	{
		throw std::exception("Failed to create {D3D11_TEXTURE2D_DESC}. Error in ctor / {device->CreateTexture2D(...)}.");
	}
	...
	if (FAILED(hr))
	{
		throw std::exception("Failed to create {DepthStencilView}. Error in ctor / {device->CreateDepthStencilView(...)}.");
	}
}